...
from TP4_LR_utils import empirical_risk, compute_accuracy
...
def GD(theta_gd, gamma, X_train, y_train, X_test, y_test, n_iterations, scikit_empirical_risk, mu):
    ...
    nb_empirical_risk_computations = 80
    ...
    for iteration in range(n_iterations):
        ...
        if iteration in n_empirical_risk_computations:
            ...
            if empirical_risk_iter < conv_tolerance*scikit_empirical_risk:
                print(f"attained 2X scikit risk in {iteration} iterations")
                break
    toc = time()
    gd_time = toc-tic
    print("exit GD")
...
def SGD(theta_sgd, gamma, X_train, y_train, X_test, y_test, n_iterations, scikit_empirical_risk, mu):
    ...
    for iteration in range(n_iterations):
        """
            EDIT ZONE START
        """
        theta_sgd = np.zeros((len(theta_sgd), 1))
        """
            EDIT ZONE END
        """
        if iteration in n_empirical_risk_computations:
...
def SAG(theta_sag, gamma, X_train, y_train, X_test, y_test, n_iterations, scikit_empirical_risk, mu):
    ...
    # log profile
    profiler.disable()
    stats_file = open(f"profiling_sag.txt", 'w')
    sys.stdout = stats_file
    profiler.print_stats(sort='time')
    # return results
    sys.stdout = sys.__stdout__
