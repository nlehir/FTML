...
\begin{definition}{Loss function}
    ...
    A \textbf{{loss function}} $l$ is an application that measures the discrepancy between to elements of a set (for instance of a linear space).
$$
l: \left\{
    \begin{array}{ll}
...
\begin{remark}
    ...
    We consider the non-centerd empirical covariance matrix:
...
\end{equation*}
    ...
    Hence in $\mathbb{R} $, $ u\mapsto u^2$ is $2$-convex. By summing linear mappings (the projections $\theta\mapsto \theta_i$) and $2$-convex applications ($\theta_i\mapsto \theta_i^2$), $\theta\mapsto ||\theta||^2$ is $2$-convex in $ \mathbb{R}^n$.
...
\begin{itemize}
    ...
    \item The bias term $B$ is equal to $0$ when $\lambda =0$ and $ \hat{\Sigma}$ is invertible. When $\lambda \rightarrow +\infty$, $B\rightarrow \theta^{*T} \hat{\Sigma}\theta^*$. It is an approximation error and does not depend on the $n$.
    \item The variance term $V$ increases in $\lambda$. It is equal to $ \frac{\sigma^2d}{n} $ when $\lambda = 0$ and $ \hat{\Sigma}$ is invertible, which corresponds to OLS. When $n\rightarrow +\infty$, $V\rightarrow 0$. It is an estimation error.
