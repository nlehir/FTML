...
def line_search(
    ...
    for iteration in range(1, n_iterations + 1):
        ...
        distance_to_opt = np.linalg.norm(theta_LS - theta_ols) ** 2
...
def gradient_descent(
    ...
    for iteration in range(1, n_iterations + 1):
        if iteration % (n_iterations // 10) == 0:
            print(f"iteration {iteration}/{n_iterations}")
        distance_to_opt = np.linalg.norm(theta_GD - theta_ols) ** 2
    ...
    if reached:
        label = r"$\gamma=$" f"{gamma}, {final_iteration} iters, {total_time:.1E} s"
        ax_linear.axvline(x=final_iteration, alpha=ALPHA, color=color)
        ax_log.axvline(x=final_iteration, alpha=ALPHA, color=color)
    else:
...
def OLS_estimator(X: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    Returns:
        theta_ols: (d, n_tests) matrix
    """
    covariance_matrix = X.T @ X
    inverse_covariance = np.linalg.inv(covariance_matrix)
    theta_ols = inverse_covariance @ (X.T @ y)
    return theta_ols
