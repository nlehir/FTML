...
from constants import SIGMA, BAYES_RISK
from utils_algo import ols_risk


def plot_test_errors_ols(risks: dict[tuple, float], n_list: list[int], d_list: list[int]):
    """
...
def plot_test_errors_ridge(risks: dict[tuple, float],
                           d_list: list[int],
                           n: int,
                           lambda_list: list[int])-> None:
    """
    Display all the computed risks on a plot
    """
    colors = ["blue", "green", "darkred", "mediumvioletred", "darkmagenta"]
    ...
    for index, d in enumerate(d_list):
        ...
        plt.plot(lambda_list, risk_estimates, "o", label=label_est, color=color, markersize=3, alpha=alpha)
        plt.plot(lambda_list, [ols_risk]*len(lambda_list), label=label_ols, color=color, alpha=alpha)


    plt.plot(lambda_list, [BAYES_RISK]*len(lambda_list), label="Bayes risk: "+r"$\sigma^2$", color="aqua")
