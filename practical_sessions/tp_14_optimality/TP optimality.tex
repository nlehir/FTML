...
\begin{equation}
    a = \argmin_{a\in \mathcal{A} } \max_{f\in \mathcal{F} } \Big[||x^k_a-x^*||\Big]
\end{equation}
...
\exercice{\textbf{}} Show that for all $l\leq d$, the components $j$ with $j\geq l+1$ are null for the iterate $x_l$, for any first-order method that produces iterates with gradients of $f$ (as defined in \ref{eq:first}). In particular, this is true for $l=k$ and for the iterate $k$, we have
...
\begin{equation}
    \frac{f(x^k)-f^*}{||x^0-x^*||^2} \geq \frac{g^*-f^*}{||x^0-f^*||^2}
\end{equation}
