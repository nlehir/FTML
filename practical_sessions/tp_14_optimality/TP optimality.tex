...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.8\linewidth]{leibniz}
    \label{fig:risks}
\end{figure}
...
\begin{equation}
    a = \argmin_{a\in \mathcal{A} } \max_{f\in \mathcal{F} } \Big[||x^k_a-x_f^*||\Big]
\end{equation}

where $x^k_a$ is the iterate returned by algorithm $a$ at iteration $k$.
...
\begin{equation}
    f(x^k)-f(x^*)\geq \frac{C}{k^2} ||x^0-x_f^*||^2
\end{equation}

where $C$ is independent on the function, then this will prove that Nesterov acceleration is optimal in $ \mathcal{A} $, as this means that for any algorithm in $ \mathcal{A} $, there exists a function, for which the iterates computed by the algorithm $a$ converge slower than $ \frac{C}{k^2} $ to its minimizer.
...
\begin{equation}
    f(x) = \frac{L}{4} \Big( \frac{1}{2} x_1^2 + \frac{1}{2} \sum^{2k}_{i=1} (x_i-x_{i+1})^2+ \frac{1}{2} x_{2k+1}^2-x_1 \Big)
\end{equation}
...
x_i^*  : \left\{
    \begin{array}{ll}
	1  - \frac{i}{2k+2} \text{ for } 1\leq i \leq 2k+1 & \\
	0 \text{ for } i\geq 2k+2
    \end{array}
\right.
$$
...
\begin{equation}
    f^* = \frac{L}{8} \big( \frac{1}{2k+2} -1\big)
\end{equation}

We consider another utility function $g$ defined as

\begin{equation}
    g(x) = \frac{L}{4} \Big( \frac{1}{2} x_1^2 + \frac{1}{2} \sum^{k-1}_{i=1} (x_i-x_{i+1})^2+ \frac{1}{2} x_{k}^2-x_1 \Big)
\end{equation}

We also admit that the minimum value of $g$, noted $g^*$, is

\begin{equation}
    g^* = \frac{L}{8} \big( \frac{1}{k+1} -1\big) 
\end{equation}
...
\exercice{\textbf{}} Show that for all $l\leq d$, the components $j$ with $j\geq l+1$ are null for the iterate $x^l$, for any first-order method that produces iterates with gradients of $f$ (as defined in \ref{eq:first}). In particular, this is true for $l=k$ and for the iterate $k$, we have

\begin{equation}
    x^k_{k+1} = x^k_{k+2} = \dots = x^k_d = 0   
\end{equation}

\exercice{\textbf{}}Show that $f(x^k)=g(x^k)$.
\\

Hence, $f(x^k)\geq g^*$ and 

\begin{equation}
    \frac{f(x^k)-f^*}{||x^0-x^*||^2} \geq \frac{g^*-f^*}{||x^0-x^*||^2} = \frac{g^*-f^*}{||x^*||^2}
\end{equation}
