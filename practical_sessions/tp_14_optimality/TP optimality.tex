...
\begin{equation}
    a = \argmin_{a\in \mathcal{A} } \max_{f\in \mathcal{F} } \Big[||f(x^t_a)-f(x_f^*)||\Big]
\end{equation}
...
\begin{equation}
    f(x_a^t)-f(x_f^*)\geq \frac{C}{t^2} ||x^0-x_f^*||^2
\end{equation}
...
\exercice{\textbf{}} Without loss of generality, we can assume that for all algorithms in $ \mathcal{A}$, the initialization is $x^0=0\in \mathbb{R}^d$: why ?
\\

We now consider that the initialization is $x^0=0$, $t\leq\frac{d-1}{2} $ and $f$ defined by
