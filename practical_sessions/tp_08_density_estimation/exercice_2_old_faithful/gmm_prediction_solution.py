...
def prediction_gmm(gmm: GaussianMixture, x: np.ndarray) -> np.ndarray:
    ...
    However, since there will two integrals (one for the expected value
    and one to approximate p(x) that appears at the denominator
    of p(x,y)/p(x)) and as these integrals are computed over the
    same range of y values, we do not need to multiply by the classical
    (b-a)/n, like when we usually approximate an integral between two real numbers
