...
def main():
    ...
    for llambda in llambda_list:
        for d in d_list:
            # Load design matrix
            X_path = os.path.join("data", f"design_matrix_n={n}_d={d}.npy")
            X = np.load(X_path)
            n = X.shape[0]
            ...
            infinity_bias = np.matmul(np.matmul(np.transpose(theta_star), Sigma_matrix), theta_star)
    ...
    title = (
        "Ridge regression: risks as a function of "
        r"$\lambda$"
        f" and d\nn={n}")
    plt.title(title)
    plt.legend(loc="best", prop={"size": 6})
    plt.tight_layout()
    figname = f"set_1_risks_n={n}_r_state_{SEED}.pdf"
