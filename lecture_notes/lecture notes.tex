...
\begin{definition}{Two times differentiable function}
    ...
    We keep the same hypotheses as in \ref{def:diff} and assume that $W= \mathbb{R}$. If the map $x\mapsto \nabla_xf(x) $ is differentiable in $x$, the we say that $f$ is two times differentiable in $x$. In that case we note $f"(x)$ the second-order derivative, that satisfies:
...
\begin{lemma}
    ...
    \begin{equation*}
	f(x+h)=f(x)+\langle\nabla_xf(x),h\rangle+ \frac{1}{2} f"(x)(h,h)+o(||h||^2)        
    \end{equation*}
\end{lemma}
...
\begin{equation*}
    \nabla_xf(x)=(L^f_x)^T\in \mathbb{R}^{d, 1}
\end{equation*}
