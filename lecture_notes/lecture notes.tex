...
\title{{Fondamentaux théoriques du Machine Learning (Cours Epita)}} 
...
\begin{proposition}
    $f$ is $ \mu$-strongly convex if and only if $ x\mapsto f(x)- \frac{\mu}{2} ||x||^2$ is convex.
\end{proposition}
...
\begin{definition}{Fixed design}
    ...
    In this setting, $X$ is assumed to be deterministic (fixed, hence the name), so the expectations are with respect to $z$ only. We note $y\in \mathbb{R}^n$ the vector that contains all the $y_i, i\in [1, n]$. $y$ is a random variable. In this context, we define the \textbf{{fixed design risk}} 
...
\begin{itemize}
	...
	% \begin{equation*}
	    % f(x) = \sum^{d}_{i=1} \theta_j\phi_j(x)
	% \end{equation*}
\end{itemize}
...
The $K$-means minimizes the distorsion by \textbf{{Expectation-maximization algorithm}}.
